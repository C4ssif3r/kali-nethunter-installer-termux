...
function set_strings() {
    ...
    if [[ ${SYS_ARCH} == "arm64" ]];
    then
        echo "[1] NetHunter ARM64 (full version)"
        echo "[2] NetHunter ARM64 (minimal version)"
        echo "[3] NetHunter ARM64 (nano version (mini-mini))"
        read -p "Enter the image you want to install ? type 1,2,3: " wimg
        if (( $wimg == "1" ));
        then
            wimg="full"
        elif (( $wimg == "2" ));
